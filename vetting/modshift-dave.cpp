...
// Declare Functions
double STDEV(double[],int),RMS(double[],int),MEDIAN(double[],int),SUM(double[],int),SUMSQ(double[],int),SELECT(double[],int,int),INVERFC(double);
...
double rms[N],sumres2[N],zparm[N],tmpdob1[N],tmpdob2[N],chi2[N]; //,runmean[N],depthfac[N];
...
double tstart,tend;
int midti;  // Index for middle of transit point
...
for(l=0;l<3;l++)  // Number of outlier passes
  {
  for(i=0;i<ndat;i++)
...
//     cout << std << endl;
  ...
  for(i=0;i<ndat;i++)
    ...
    k=0;
    for(j=0;j<ndat;j++)
      if(fabs(data[0][i]-data[0][j]) < 0.1*width)  // look at points within a transit duration
        {
        tmpdob1[k]=data[3][j];
        k++;
        }
...
for(i=0;i<ndat;i++)  // Calculate chi squared and find lowest chi squared value
  ...
  if(chi2[i] < chi2low)
    chi2low = chi2[i];
  outfile << fixed << setprecision(10) << data[0][i]/period << " " << data[1][i] << " " << data[2][i] << endl; // Write out original data while we're at it
