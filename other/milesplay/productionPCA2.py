...
def chooseBestSGcdppValue(sgcdppList, evalues, thresh=0.5):
...
def generatePrinComps(dataCube, thrusterFireInds):
    ...
    numPixels = dataCube[0].size

    # get the total light curve by summing all of the cadences
    totLightcurve = getRawLightcurve(dataCube, thrusterFireInds)
...
def chooseNumPrinComps(prinComps, totLightcurve, numPixels, evalues):
    ...
    optimalNumPC = chooseBestSGcdppValue(sgcdppList, evalues)
    
    return optimalNumPC
    
def pixSeriesPrep(dataCube, thrusterFireInds):
    ...
    return pixSeries, pixSeriesNorm
