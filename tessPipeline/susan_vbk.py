...
def createConfig(detrendType, sector, tic, planetNum, debugMode=True):
    cfg = dict()
    cfg['debug'] = debugMode
    cfg['sector'] = sector
    cfg['campaign'] = sector
    ...
    cfg['value'] = tic
    cfg['detrendType'] = detrendType#"tess"#"eleanor"#

    #TODO This shouldn't be hardcoded, but passed as a parameter
    cfg['dvtLocalPath'] = "/Users/vkostov/Desktop/Ideas_etc/DAVE_test/TESSting/justVet/"
    ...
    cfg['modshiftBasename'] = "/Users/vkostov/Desktop/Ideas_etc/DAVE_test/TESSting/justVet/"#tmp"       
...
def runOneDv(detrendType, sector, tic, planetNum, debugMode=True):
    
    cfg = createConfig(detrendType,sector,tic,planetNum,debugMode=True)
...
def runAllTces(tceFile,sector,outfile):
    ...
    for index,row in df[37:38].iterrows():
        ...
        clip = runOneDv(detrendType, sector,row.ticid,row.planetNumber)
