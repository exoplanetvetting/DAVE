...
def createConfig(detrendType, sector, tic, planetNum, period, tepoch, tdepth, tdur, debugMode=True):#(sector,tic,planetNum,debugMode=True)
    cfg = dict()
    cfg['debug'] = debugMode
    cfg['sector'] = sector
    cfg['campaign'] = sector
    ...
    cfg['taskList'] = ['serveTask', 'detrendTask', 'trapezoidFitTask','modshiftTask', 'sweetTask', 'lppMetricTask', 'centroidsTask']
...
def runOneDv(detrendType , sector, tic, planetNum, period, epoch, tdepth, tdur, debugMode=True):
	...
	if prob > minProbForFail:
		...
		rv['isCandidate'] = False
		rv['reasonForFail'] = "Centroid offset probability is %.1e" %(prob)
    	else:
