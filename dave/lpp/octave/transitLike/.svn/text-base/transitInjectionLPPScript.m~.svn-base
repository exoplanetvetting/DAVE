...
other(1).dir='/soc/nfs/so-nfs/Q17-Injection-NEW/TrapezoidFitPDC/DATS/';
other(2).dir='/soc/nfs/so-nfs/Q17-Injection-NEW/DVSummaries/DATS/';
other(1).name='TrapDet'; %Do not change
other(2).name='DVMedDet'; %Do not change
other(1).tag='-fulltime.dat';
other(2).tag='-fulltime.dat';

storeDir='/home/sethomps/Kepler/RoboVetter/Q1Q17knnlpp/TrInjection2/TrInDataStore/';
tceList='TrInTces.txt';  %Input File
%tceList='difftcelist.txt';
dvFileList='TrInGetDvPath.txt';  %Run Fergal's code to get this.
...
outfile=['inputTrInQ1Q17DR24_Matrix',num2str(datenum(date)-730000),'.mat'];
save(outfile,'infoTrap','infoPdc','infoDV')
...
%
%
origMapFile='../ProductionRun/mappingQ1Q17DR24dv_6083.mat';
load(origMapFile);
origMap=outputInfo;
...
savefile=['TrInQ1Q17DR24_LPP',info.dettype,num2str(datenum(date)-730000),'.dat'];
trInInfoDV=computeTransitinessKnownMap(info,origMap, savefile);

outfile=['TrInQ1Q17DR24_LPP',info.dettype,num2str(datenum(date)-730000),'.mat'];
save(outfile,'trInInfoDV')
%%
%Step three: Gather the right information to apply the same mapping and
%comparision
%
%
origMapFile='../ProductionRun/mappingQ1Q17DR24trap_6084.mat';
load(origMapFile);
origMap=outputInfo;
%These lines can be removed for future runs.
%types=origMap.d;
...
savefile=['TrInQ1Q17DR24_LPP',info.dettype,num2str(datenum(date)-730000),'.dat'];
trInInfoTrap=computeTransitinessKnownMap(info,origMap, savefile);

outfile=['TrInQ1Q17DR24_LPP',info.dettype,num2str(datenum(date)-730000),'.mat'];
save(outfile,'trInInfoTrap')
%%
